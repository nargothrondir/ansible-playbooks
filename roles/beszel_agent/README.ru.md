...
|------------|--------------|----------|
| `beszel_agent_version` | `0.20.0` | Версия бинарника для первой установки (дальше его обновляет таймер). |
| `beszel_agent_auto_update` | `true` | Ежедневный таймер `beszel-agent update` — то же, что создаёт установщик Beszel на вопрос «включить автообновление». Сверяет SHA-256 релиза (с 0.18.8) и перезапускает агента. `false` убирает таймер. |
| `beszel_agent_update_calendar` | `daily` | `OnCalendar` таймера. |
| `beszel_agent_update_random_delay` | `4h` | `RandomizedDelaySec` таймера, чтобы ноды не обновлялись в одну минуту. |
