...
| `beszel_agent_system_label` | `{{ beszel_agent_system_name }}` | Имя, с которым создаётся НОВАЯ система в хабе (провижининг передаёт, например, `3.1 FI Node [Провайдер]`). Только при создании — переименовать в хабе потом можно. |
