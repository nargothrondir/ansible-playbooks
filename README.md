...
| [playbooks/migrate-inbound-tags.yml](playbooks/migrate-inbound-tags.yml) | — (panel API; renames one profile's inbounds to `<CC>_VLESS_TCP_REALITY` / `<CC>_VLESS_XHTTP_TLS` and re-binds squads, nodes and hosts) | control |
